...
df_ages_years_all <- read_csv("data/oldbailey_defendant_all.csv") #Reading the data-sheet
...
df_ages_years_all <- df_ages_years_all %>%  #Creating four age-groups
  mutate(age_group_1_21 = rowSums(df_ages_years_all[, 3:23])) %>% 
  mutate(age_group_22_30 = rowSums(df_ages_years_all[, 24:32])) %>% 
  mutate(age_group_31_50 = rowSums(df_ages_years_all[, 33:52])) %>% 
...
df_ages_years_all <- df_ages_years_all %>%
  mutate(total = rowSums(df_ages_years_all[, 3:6])) 


## 1800-1913 men
df_men <- read_csv("data/oldbailey_defendant_males.csv")
df_men <- df_men %>% rename(year = key)
df_men <- filter(df_men, year >= 1800)

df_men <- df_men %>%  #Creating four age-groups
  mutate(age_group_1_21 = rowSums(df_men[, 3:23])) %>% 
  mutate(age_group_22_30 = rowSums(df_men[, 24:32])) %>% 
  mutate(age_group_31_50 = rowSums(df_men[, 33:52])) %>% 
...
df_men <- df_men %>%
  mutate(total = rowSums(df_men[, 3:6])) 


## 1800-1913 women
df_women <- read_csv("data/oldbailey_defendant_females.csv")
df_women <- df_women %>% rename(year = key)
df_women <- filter(df_women, year >= 1800)

df_women <- df_women %>%  #Creating four age-groups
  mutate(age_group_1_21 = rowSums(df_women[, 3:23])) %>% 
  mutate(age_group_22_30 = rowSums(df_women[, 24:32])) %>% 
  mutate(age_group_31_50 = rowSums(df_women[, 33:52])) %>% 
...
df_women <- df_women %>%
  mutate(total = rowSums(df_women[, 3:6])) 


## Punishment categories
df_punish <- read.csv("data/oldbailey_defendant_punishments.csv")
df_punish <- df_punish %>% rename(year = key)
df_punish <- filter(df_punish, year >= 1800)


## Offence categories
df_offence <- read.csv("data/oldbailey_defendant_offences.csv")
...
dfplot_ages_years_all <- select(df_ages_years_all, year, age_group_1_21, age_group_22_30, age_group_31_50, age_group_51_99, total)
...
ggplot(dfplot_ages_years_all, aes(x = year, y = executions, color = age_group)) +
  ...
  theme_minimal() +
  scale_color_manual(values = c("age_group_1_21" = "blue", "age_group_22_30" = "green", "age_group_31_50" = "red", "age_group_51_99" = "yellow", "total" = "black"), labels = c("1-21 years", "22-30 years", "31-50 years", "51-99 years", "Total")) +
...
dfplot_men <- select(df_men, year, age_group_1_21, age_group_22_30, age_group_31_50, age_group_51_99, total)
...
dfplot_women <- select(df_women, year, age_group_1_21, age_group_22_30, age_group_31_50, age_group_51_99, total)
